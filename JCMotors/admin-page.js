...
function handleSubmit(event) {
  ...
  reader.onload = function () {
    ...
    // Reset the form
    form.reset();
    alert("You have successfully added a new vehicle!");
  };
}
...
function updateUI(items) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const itemRow = `
      <div class="border rounded postSelf">
        <div class=''><img src="${item.image}" draggable="false"></div>
        <div class='fw-bold fs-5'>${item.name}</div>
        <div class=''> ${item.price} <small class='fw-bold'>per hour</small></div>

        <div><button class='btn btn-secondary' onclick="deleteItem(${i})">Remove Vehicle</button></div>
      </div>
    `;
